...
import { resolveResourceLoaderOriginContract } from '../../tools/resource-loader-contract.mjs';
...
assert.equal(
  resourceLoaderContract.shared.hostSurfaces.parserOutput,
  ':where([data-tt-vector-surface="parser-output"])'
);
assert.equal(resourceLoaderContract.shared.ownershipPolicies.skin.isolateHostContent, true);
assert.equal(resourceLoaderContract.shared.ownershipPolicies.skin.admittedSurface, 'parserOutput');
assert.equal(resourceLoaderContract.pageStyleQueue.profile, 'vector-legacy-projection');
assert.match(
  generatedVectorCss,
